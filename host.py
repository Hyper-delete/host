...
# --- Configuration ---
TOKEN = "8610775864:AAF3Gv6awXaOJp4hNwzbXtJ0s-qFH8z771g" #bot token dalo yeha
...
referral_enabled = True  # Owner can toggle referral system on/off
...
def record_violation(user_id, username, filename, reason, bot, owner_id=7623391678):
    ...
    try:
        ...
        c.execute('''
            INSERT INTO security_logs (timestamp, user_id, username, filename, reason, violation_count)
            VALUES (?, ?, ?, ?, ?, ?)
        ''', (timestamp, user_id, username, filename, reason, violation_count))
